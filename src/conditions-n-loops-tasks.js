...
function isIsoscelesTriangle(a, b, c) {
  ...
  const ac = a + c;
  const cb = c + b;
  const minSum = Math.min(ab, ac, cb);
  const maxLine = Math.max(a, b, c);
  return minSum > maxLine ? a === b || a === c || b === c : false;
}
...
function convertToRomanNumerals(num) {
  let ansStr = '';
  const maxDecade = Math.floor(num / 10);
  for (let i = 0; i < maxDecade; i += 1) {
    ansStr += 'X';
  }
  ...
  if (reminder >= 5) {
    ansStr += 'V';
    const countUnit = reminder % 5;
    for (let i = 0; i < countUnit; i += 1) {
      ansStr += 'I';
    }
  } else {
    for (let i = 0; i < reminder; i += 1) {
      ansStr += 'I';
    }
  }
  return ansStr;
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    if (i !== 0 && i !== numberStr.length) ansStr += ' ';
    switch (numberStr[i]) {
      case '0':
        ansStr += 'zero';
        break;
...
function isContainNumber(num, digit) {
  ...
  while (flagEnd) {
    ...
    if (Math.floor(newNum / 10) === 0) flagEnd = false;
...
function getSpiralMatrix(size) {
  const resultMat = [];
  for (let i = 0; i < size; i += 1) resultMat[i] = [];
  let valueRow = 0;
  let valueColumn = 0;
  let changeDirectionCount = size - 1;
  let changeDirectionFlag = true;
  let x = 1;
  let speedX = 1;
  let speedY = 1;
  for (let i = 1; i <= size * size; i += 1) {
    resultMat[valueRow][valueColumn] = i;
    ...
    if (changeDirectionCount === 0 && changeDirectionFlag) {
      changeDirectionCount = size - x;
